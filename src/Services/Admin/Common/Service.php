...
abstract class Service {
        ...
        foreach ($columns as $column) {
            ...
            if ($column['type'] !== 'hidden') {
                ...
                if (empty($column['title'])) {
                    $key = "model/{$this->table}.{$column['name']}";

                    $column['title'] = i18n($key);

                    if ($column['title'] === $key) {
                        $column['title'] = i18n("model/default.{$column['name']}");
                    }
                }
            }
    ...
    protected function getBreadcrumbs($items, $context) {
        $breadcrumbs = [];
        ...
        if ($menu) {
            ...
            while ($menu) {
                if (empty($menu['tag'])) {
                    ...
                    $path = $this->render($path, $context);
                    $context = data_get($items, count($breadcrumbs));
                    $breadcrumbs[] = ['label' => $context?->toTitle(), 'path' => $path, 'title' => i18n($menu['title'])];
                }
    ...
    private function render($template, $context) {
        return preg_replace_callback('/\{(\w+)\}/', fn ($matches) => $context[$matches[1]] ?? $matches[0], $template);
    }
