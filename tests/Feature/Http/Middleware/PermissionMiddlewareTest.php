...
class PermissionMiddlewareTest extends FeatureTestCase {
     ...
     */
    private function token(int $id, ?int $groupId = null, array $permissions = []): string {
        return UserFactory::new()->createOne(['id' => $id, 'group_id' => $groupId, 'permissions' => $permissions])->createToken();
    }
    ...
    public function test_a_granted_regular_user_reaches_the_endpoint(): void {
        $this->withToken($this->token(2000, null, ['user' => ['query' => true]]))
            ->postJson('admin/user')
            ->assertJsonPath('reached', true);
    }
